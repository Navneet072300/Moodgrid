...
export function Header() {
  ...
  return <>
    <header className="app-header"><div className="header-inner">
      ...
      <div className="header-actions"><ThemeCustomizer /><details className="account-menu"><summary aria-label="Account menu"><span className="avatar">{profile.username.slice(0, 1).toUpperCase()}</span><ChevronDown size={14} /></summary><div className="account-popover"><strong className="block truncate text-sm">@{profile.username}</strong><p className="truncate text-sm text-muted">{demo ? "Your demo journal" : email}</p>{encrypted && <button onClick={lock} className="text-link"><LockKeyhole size={15} />Lock journal</button>}<Link href={demo ? "/demo/account" : "/account"} className="text-link">Edit profile →</Link><Link href={demo ? "/demo/stickers" : "/stickers"} className="text-link">My stickers →</Link>{demo ? <Link href="/login" className="text-link">Start your own journal →</Link> : <button onClick={logout} disabled={pending} className="flex items-center gap-2 text-sm"><LogOut size={16} />{pending ? "Signing out…" : "Sign out"}</button>}{error && <p role="alert" className="error-message">{error}</p>}</div></details></div>
    </div></header>
